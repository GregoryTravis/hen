...
(define (match-rewrite env body)
  (cond
   ...
   ((null? body) body)
   ((pair? body)
    (cons (match-rewrite env (car body))
          (match-rewrite env (cdr body))))
   ((literal? body) body)
...
(define (match-render target clauses all-clauses)
  (if (null? clauses)
      ...
      (begin
        ...
        (let* ((pat (car clauses))
               (body (cadr clauses))
               (env-var (match-vargen)))
          `(let ((,env-var (match-target ',pat ,target)))
             (if (eq? #f ,env-var)
                 ,(match-render target (cddr clauses) clauses)
                 (match-rewrite ,env-var ',body)))))))

(define (match-target pat target)
  (let ((env (match-target0 pat target)))
    (if (not (null? (grep (lambda (x) (eq? x #f)) env)))
        #f
        env)))

(define (match-target0 pat target)
  (cond
   ((and (pair? pat) (pair? target))
    (append (match-target0 (car pat) (car target))
            (match-target0 (cdr pat) (cdr target))))
   ((eq? pat '_) '())
